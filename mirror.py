...
# ]
# ///
"""Update fortitude-pre-commit to the latest version of fortitude."""
...
def get_all_versions() -> list[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/fortitude-lint/json")
...
def get_current_version(pyproject: dict) -> Version:
    ...
    requirement = next((r for r in requirements if r.name == "fortitude-lint"), None)
    assert requirement is not None, "pyproject.toml does not have fortitude-lint requirement"

    specifiers = list(requirement.specifier)
    assert (
        len(specifiers) == 1 and specifiers[0].operator == "=="
    ), f"fortitude-lint's specifier should be exact matching, but `{requirement}`"
...
def process_version(version: Version) -> typing.Sequence[str]:
    def replace_pyproject_toml(content: str) -> str:
        return re.sub(r'"fortitude-lint==.*"', f'"fortitude-lint=={version}"', content)
